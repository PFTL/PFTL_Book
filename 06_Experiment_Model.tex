\chapter{Writing The Experiment Model}\label{writing-our-experimentmodel}
...
A model for experiments will allow you to quickly establish the
...
\mintinline{python}{user}, a \mintinline{python}{daq}, and \mintinline{python}{properties}. The user is set
to \mintinline{python}{None} by default, but it is useful to keep track of who has
...
\mintinline{python}{getpass.getuser()}.
...
a \mintinline{python}{properties} dictionary. If you remember from some chapters
...
At this point is important to show what kind of functionality we are
...
\mintinline{python}{'Me'}. Then you specify where
...
\mintinline{python}{do_scan}. In principle, the model could have a quick way of
...
\exercise{Write a method for loading the properties from a {YAML} file. Remember
that you have already done a version of this in the Lay Out chapter.}
...
\exercise{Write a \mintinline{python}{do_scan} method that relies on the parameters stored at
\mintinline{python}{properties} to perform a measurement. Assume that
\mintinline{python}{self.daq} is the model for the daq you are going to use. Store
the data of the measurement in a property of the experiment class,
remember that you should add them to the \mintinline{python}{__init__}.}
...
\exercise{Write a method for saving the metadata of your experiment, i.e., all the
information stored in the \mintinline{python}{self.properties} dictionary.
\textbf{TIP}: Where do you save the data? Perhaps you can add one more
key to the original {YAML} file to specify the destination folder.}
...
device you are using in the \mintinline{python}{properties} dictionary, that
...
method \mintinline{python}{load_daq} will look like this:
