...
We are going to start with the simplest approach, which consists of importing and initializing the daq outside of the experiment model and then loading it. The method \mintinline{python}{load_daq} will look like this:
...
argument, \texttt{daq}, which is in turn stored as the class attribute \mintinline{python}{self.daq}.
...
There are, however, few things that need to be addressed. First, let's check what happens on the line which reads \mintinline{python}{str(self.daq)}. Remember, every time you see something with an unclear behaviour, you can always check what is going on by opening a terminal and starting Python. Then, we can do the following:
...
The result of doing \mintinline{python}{str(daq)} may not be what you were expecting. It is, nonetheless, a good starting point because it allows you to identify which class you used. Fortunately, this can be improved. Open the file where you store the \mintinline{python}{AnalogDaq} class and add the following to it:
...
\mintinline{python}{__str__} method of a class tells Python how to convert an object to a string. It is also responsible for the behaviour of \mintinline{python}{print(daq)}. Now you will have a nicely formatted string to store in your \mintinline{python}{properties} dictionary. Remember that if you assign the same output for the \mintinline{python}{__str__} method, to two different classes, you won't be able to distinguish which one you used. Therefore, deciding what to include in the string representation requires some thinking. You can also add something that identifies that specific DAQ such as its serial number or the port to which it is connected. 
...
The \mintinline{python}{isinstance} function is very useful to check whether the object we are inspecting belongs to a special class, including parents. The function works also with default data types of Python, for example:
...
method both as \mintinline{python}{load_daq()} and as \mintinline{python}{load_daq(daq)}. If you call the method without an argument, \mintinline{python}{if daq is None} will be \mintinline{python}{True}. The way of importing the different models is quite straightforward, you check the name specified in the config file and make the import. Pay attention to the fact that we added another fail-safe: if the user does not supply an initialized DAQ nor any DAQ information in the configuration file, we will raise an exception to inform it. You could argue that this approach is not very flexible. If you ever add a model for a new device, you need to manually add it to the experiment model. However, the simplicity outweighs the flexibility. 
...
generate an output and record an input within a \emph{for loop} with the range given by \mintinline{python}{start}, \mintinline{python}{stop} and \mintinline{python}{step}. You should have something like this:
...
\exercise{Update the DAQ model to use {Q_} instead of importing Pint and generating a unit registry in the model file.}

\note{By defining \texttt{Q_} at the root of the project, we are actually hiding a problem that you may encounter in other projects. The unit registry is a complex object, in which you specify how units relate to each other and it allows you to define your own units if needed. This means that different unit registries cannot be mixed together. If you define the unit registry in the experiment model and in the daq model, at some point you will see an exception warning you about mixing different unit registries. It is good practice to create only one unit registry per project.}
...
When we apply \texttt{int} to a quantity, Pint automatically tries to transform the variable to a dimensionless quantity. This is exactly the case when we divide volts and millivolts. The same is valid if we use \textttt{float}, for example. However, if you use \texttt{int} to a Quantity with units, it will raise an exception. The number of points is not an issue for performing the scan. There is another parameter which we need to take care of. The \mintinline{python}{delay} has units of time, and the function \mintinline{python}{sleep} takes seconds as an argument. The easiest way to deal with it is to simply transform the delay to the units needed by the function:

\begin{minted}{python}
sleep(delay.m_as('s'))
\end{minted}

Which ensures that you use the delay magnitude expressed in seconds.  If you use milliseconds or hours for that matter, it will be translated into the proper number before the \mintinline{python}{sleep} function uses it. With these changes, your code is almost ready for performing a scan. 
...
A way around this problem is to select the units we want to work with before using \texttt{linspace}. There is no problem with this approach, because in any case the DAQ model will take care of translating the units before sending the command to the device. We can to the following:
...
You can see now that the last value is beyond the range we set.
...
\exercise{Add an \texttt{initialize} method into the base class.}
...
\exercise{Define a new method, called \mintinline{python}{monitor_signal} which uses the parameters defined in the previous exercise to continuously acquire a value. If you want to test how the monitor works, you can make the loop stop after a certain amount of time, for example using the module \mintinline{python}{time.time()}.\\ 
\textbf{{HINT}}: You should define a numpy array where to store the data. In order to move all the values one step to the left (or right), you can use \mintinline{python}{np.roll}. It is always a good exercise to look up the documentation online to understand how Numpy functions work.}
