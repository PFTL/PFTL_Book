...
We are going to start with the simplest approach, which consists of importing and initializing the daq outside of the experiment model and then loading it to the \emph{model}. The method \mintinline{python}{load_daq} will look like this:
...
argument, \texttt{daq}, which is in turn stored as the class property \mintinline{python}{self.daq}.
...
can be improved. Open the file \textbf{dummy\_daq.py} and add the following to the \mintinline{python}{DummyDaq}
class:
