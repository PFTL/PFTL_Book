...
\exercise{In the loop, the variable \mintinline{python}{data} is being overwritten on every step. This is not what you want if you intend to do anything with it. Store the information into an array. The array should be an attribute of the class, i.e. it should be defined in the \mintinline{python}{__init__} method and you can refer to it as \mintinline{python}{self.NAME_OF_VARIABLE}.}
...
Once you complete the exercise above, one of the problems was addressed. But still, if you try to run the program you will face other issues. For example, to calculate the number of points in the scan,
we should transform all the parameters to the same units. One of the possible ways of doing it is by picking the units of one of the parameters, like this:
...
Which ensures that you use the delay magnitude expressed in seconds. 

!!!!

If you use milliseconds or hours for that matter, it will be translated into the proper number before the \mintinline{python}{sleep} function uses it. With this changes, your code is ready for performing a scan. There are some concerns, such as how do you stop a scan while it is running. But the core ideas and methods are already there.
