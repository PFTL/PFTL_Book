...
To load the DAQ into your experiment, you can start with the simplest approach. You import the model for the daq, and you initialize it using the \py{load_daq} method. You can update the experiment model (we omitted some parts of the code for brevity):
...
The method \py{__str__} is called a \emph{dunder} method, because it has the double underscore before and after its name. These are magic methods in classes that allow you to change the behavior at a much lower level. The \emph{str} method is responsible for letting Python know how to transform an object to a string. It happens, for example, when you use the \py{print} function. If you rerun the experiment, you see that the output is clearer than before.

\infoInfo{String Representation}{Adding a string representation to the classes you build is an excellent addition but not mandatory. You should be careful not to derail on details that don't bring you close to the goal of performing a measurement.}
