...
In the example above, you've defined a class called \mintinline{python}{Person} that doesn't do anything (that is why it says \mintinline{python}{pass}.) You can add more functionality to this class by declaring a function that belongs to it. Create a file called \textbf{person.py} and add the following code to it:
...
In Python, the functions that belong to classes are called \textbf{methods}. For using the class, you have to create a variable of type \py{person}. Back in the Python Interactive Console, you can, for example, do this:
...
The first line imports the code into the interactive console. For this to work, you must start Python from the same folder where the file \textbf{person.py} is located. When you run the code above, you should see as output \mintinline{python}{John Snow}. One important detail has been omitted: the presence of \mintinline{python}{self} in the declaration of the method. All the methods in Python take a first input variable called \py{self}, referring to the class itself. For the time being, don't stress about it, but bear in mind that when you define a new method, you should always include the \mintinline{python}{self}; when calling the method, you should \emph{never} include it. You can also write methods that don't take any input, but still have the \mintinline{python}{self} in them, like this for example:
...
What you can see in this example is that the method \mintinline{python}{store_name} takes one argument, \mintinline{python}{name}, and stores it in the class variable \mintinline{python}{stored_name}. As with methods, variables are called \textbf{properties} in the context of a class. The method \mintinline{python}{get_name} just returns the stored property. What you see in the last line is that you can access the property directly without needing to call the \mintinline{python}{get_name} method. In the same way, you don't need to use the \mintinline{python}{store_name} method:
...
If you go ahead and run the \mintinline{python}{get_name} without actually storing a name beforehand, now there is no error, as it just returns an empty string. While initializing, you can also force the execution of other methods, for example:
...
It has the same final effect. It is a common (and smart) practice to declare all the variables of your class at the beginning, inside your \mintinline{python}{__init__} method. In this way, you won't depend on calling specific methods to create the variables.
...
When you do this, your previous code stops working, because now you have to set the \mintinline{python}{name} explicitly. If there is any other code that does \mintinline{python}{Person()}, it fails. The proper way of altering the functioning of a method is to add a default value in case no explicit value is passed. The \py{__init__} would become like so:
...
With this modification, if you don't explicitly specify a name when instantiating the class, it defaults to \py{''}, that is, an empty string.
...
So far, if you wanted to have properties available right after the instantiation of a class, you had to include them in the \py{__init__} method. However, this is not the only possibility. You can define properties that belong to the class itself. Doing so is as simple as declaring them before the \py{__init__} method. For example, you could do the following:
...
If you use the new \py{Person} class, you will have a property called \py{birthday} available, but with some interesting behavior. Let's see! First, you start as always:
...
What you see above is that it doesn't matter if you define the birthday within the \py{__init__} method or before it; when you instantiate the class, you access the property in the same way. The main difference is what happens before instantiating the class:
...
One of the advantages of working with classes in Python is that it allows you to use the code from other developers and expand or change its behavior without modifying the original code. The best thing would be to see it in action. So far, you have a class called \py{Person}, which is generally not too useful. Let's assume you want to define a new class, called \py{Teacher}, that has the same properties as a \py{Person} (that is, a name and a birthday), in addition to having the ability to teach a class. You can add the following code to the file \textbf{person.py}:
...
Note that in the definition of the new \py{Teacher} class, you have already added \py{Person}. In Python jargon, this means that the class \py{Teacher} is a child of the class \py{Person} (or vice versa, that \py{Person} is the parent of \py{Teacher}). This is called \textbf{inheritance} and is not only very common in Python programs, it is also one of the characteristics that makes Python so versatile. You can use the class \py{Teacher} in the same way as you have used the class \py{Person}:
...
However, there is also another approach to avoid the error. You could simply run the \py{__init__} method of the parent class (that is, the base class) by adding the following:
...
When you use \py{super()}, you are going to have direct access to the class from which you are inheriting. In the example above, you explicitly called the \py{__init__} method of the parent class. If you try to run the method \py{me.get_name()} again, you'll see that no error appears, but also that nothing appears on the screen. This is because you triggered the \py{super().__init__()} without any arguments, and therefore the name defaulted to the empty string.
...
You can get very creative. It is also important to note that the method \py{__str__} is used when you want to transform an object into a string, for example:
...
Something important to point out is that this method is inherited. Therefore, if instead of printing a \py{Person}, you print a \py{Student}, you'd see the same output, which may or may not be the desired behavior.
...
There is a way of doing this in Python which can be easily implemented even if you don't fully understand the syntax. Working again in the class \py{Person}, you can do the following:
\begin{minted}{python}
class Person():
...
Python gives you control over everything, including what the assignment operator \py{=} does when you assign a value to an attribute of a class. The first time you create a \py{@property}, you need to specify a function that returns a value. In the case above, you are returning \py{self._year_of_birth}. Just doing that allows you to use \py{me.year_of_birth} as an attribute, but it fails if you try to change its value. It is called a read-only property. If you are working in the lab, it is useful to define methods as read-only properties when you can't change the value. For example, a method for reading the serial number would be read-only.
